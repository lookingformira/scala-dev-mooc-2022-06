...
object homework_hkt_impllicts{
    ...
    def tupleF[F[_], A, B](fa: F[A], fb: F[B])(implicit bindableA: Bindable[F, A], bindableB: Bindable[F, B]): F[(A, B)] =
      Bindable.tupleBindable(bindableA, bindableB)
  ...
  object Bindable {
    ...
    implicit def listBindable[A](implicit list: List[A]): Bindable[List, A] = new Bindable[List, A] {
      override def map[B](f: A => B): List[B] = list.map(f)

      override def flatMap[B](f: A => List[B]): List[B] = list.flatMap(f)
    }
    implicit def optBindable[A](implicit option: Option[A]): Bindable[Option, A] = new Bindable[Option, A] {
