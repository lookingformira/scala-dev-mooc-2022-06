...
object opt {
  ...
  trait Option[+T] {
     ...
     */
    def zip[B](opt: Option[B]): Option[(T, B)] = for {
      firstVal <- this
      secondVal <- opt
     ...
     */
    def filter(condition: T => Boolean): Option[T] =
      this.flatMap { t =>
        if (condition(t)) Option.Some(t) else Option.None
      }
...
object list {
  ...
  trait List[+T] {
    ...
    def mkString(separator: String): String = {
      ...
      @tailrec
      def loop(acc: String, list: List[T]): String = list match {
        case List.Nil => acc
        case List.::(head, tail) => if (acc.isEmpty) loop(s"$head", tail) else loop(s"$acc$separator $head", tail)
      }
    ...
    def reverse: List[T] = {
      @tailrec
      def loop(acc: List[T], oldList: List[T]): List[T] = oldList match {
        case List.Nil => acc
        case List.::(head, tail) => loop(acc.::(head), tail)
      }
     ...
     */
    def map[B](func: T => B): List[B] = {

      @tailrec
      def loop(acc: List[B], oldList: List[T]): List[B] = oldList match {
        case List.Nil => acc
        case List.::(head, tail) => loop(acc.::(func(head)), tail)
      }
    ...
    def filter(condition: T => Boolean): List[T] = {
      ...
      def loop(acc: List[T], oldList: List[T]): List[T] = oldList match {
        case List.Nil => acc
        case List.::(head, tail) => if (condition(head)) loop(acc.::(head), tail) else loop(acc, tail)
      }
  ...
  object List {
     ...
     */
    def fill[T](number: Int)(value: T): List[T] = {

      @tailrec
      def loop(acc: List[T], n: Int): List[T] =
        if (n <= 0) acc else loop(acc.::(value), n - 1)

      loop(List.Nil, number)
    }
     ...
     */
    def shoutString(list: List[String]): List[String] = list.map(string => s"!$string")
